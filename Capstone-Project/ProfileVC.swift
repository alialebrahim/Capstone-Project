...
class ProfileVC: UIViewController, ProviderInfoDelegate, PredefinedServicesDelegate {
    ...
    func viewControllerForSelectedSegmentIndex(index: Int) -> UIViewController? {
        ...
        switch index {
        case 0 :
            if let vc = self.storyboard?.instantiateViewControllerWithIdentifier("providerInfoVC") {
                return vc
            }
        case 1 :
            if let vc = self.storyboard?.instantiateViewControllerWithIdentifier("providerPredefinedServicesVC") {
                return vc
            }
        default:
            break
...
//            navigationItem.rightBarButtonItem = nil
            if tabIndex == 0 {
                ...
                navigationItem.title = "Username"
                let myVC = vc as! ProviderInfoVC
                let editProfileItem = UIBarButtonItem(barButtonSystemItem: .Edit, target: self, action: #selector(ProfileVC.editProfileAction))
                navigationItem.rightBarButtonItem = editProfileItem
                myVC.delegate = self
            }else if tabIndex == 1{
                navigationItem.title = "Services"
                let myVC = vc as! PredefinedServicesVC
                let addServiceItem = UIBarButtonItem(barButtonSystemItem: .Add, target: self, action: #selector(ProfileVC.addNewServiceAction))
